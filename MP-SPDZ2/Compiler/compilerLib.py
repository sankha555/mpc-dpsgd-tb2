...
class Compiler:
    ...
    def remote_execution(self, args=None):
        ...
        for host in hosts:
            ...
            host_split = split[0].split(':', maxsplit=1)
            hostnames.append(host_split[0])
            port_list.append(int(host_split[1]))
            if len(split) > 1:
                destinations.append(split[1])
            else:
                destinations.append('.')
        #connections = [Connection(hostname) for hostname in hostnames]
        connections = [Connection(host=hostnames[i], port=port_list[i]) for i in range(len(hostnames))]
